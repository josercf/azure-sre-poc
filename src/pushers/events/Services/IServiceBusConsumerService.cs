...
public class ServiceBusConsumerService : IServiceBusConsumerService
{
        ...
        IWebhookService webhookService)
    {
        ...
        _subscriptionName = configuration["AzureServiceBus:EventsSubscriptionName"];
        ...
        try
        {
            ...
            // Create processor for the subscription with filter for events (skills 1-3)
    ...
    private async Task ProcessMessageAsync(ProcessMessageEventArgs args)
    {
        ...
        try
        {
            ...
            // Process the message
            using var processActivity = _tracing.StartProcessActivity("ProcessEvents", message);
            ...
            try
            {
                ...
                // Filter for events (skills 1-3)
                if (championshipData.IdSkill >= 1 && championshipData.IdSkill <= 3)
                {
                    ...
                    // Send to webhook
                    using var webhookActivity = _tracing.StartWebhookActivity("PushEventsWebhook", championshipData);
                    ...
                    try
                    {
                        ...
                        var success = await _webhookService.SendChampionshipDataAsync(championshipData, webhookUrl);
                ...
                {
                    // Not an event skill, abandon the message
                    await args.AbandonMessageAsync(message);
                    _logger.LogInformation("Message abandoned - not an event skill (skill={Skill}): {MessageId}", 
